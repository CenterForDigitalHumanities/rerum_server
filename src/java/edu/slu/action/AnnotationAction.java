...
//            BasicDBObject asbdbo = (BasicDBObject) asdbo;
     ...
     * 
     * @param objectToCheck A JSON object to parse through and validate.  It is considered a "Top Level Object".
     * @param action A string that is either "save" or "update" so we know what action is being performed
     * @return compliant A JSONObject filled with warnings, errors and a valid flag.
     */
    public JSONObject checkIIIFCompliance(JSONObject objectToCheck, String action){
        JSONObject compliant = new JSONObject();
        JSONArray warnings = new JSONArray();
        JSONArray errors = new JSONArray();
        JSONObject error_or_warning = new JSONObject();
        String context = "";
        String type = "";
        String version = "0";
        int valid = 0;
        ...
        try{
            context = objectToCheck.getString("@context");
            if(context.contains("/presentation/3")){
                version = "3";
            }
            else if(context.contains("/presentation/2")){
                version = "2"; //people using http://iiif.io/api/presentation/2/context.json are presumably 2.1, but you could check that they are valid 2.0
            }
            else if(context.contains("www.shared-canvas.org/ns/context.json")){
                version = "1";
            }
            else{
                version = "custom_context";
            }
        }
        catch (Exception e){ //Did not have @context
            error_or_warning.element("error_"+errorCounter, "No @context found.");
            errors.add(error_or_warning);
            error_or_warning.clear();
            errorCounter++;
        }
        try{
            type = objectToCheck.getString("@type");
        }
        catch (Exception e){ //Did not have @type, this is an error.  
            error_or_warning.element("error_"+errorCounter, "No @type found.");
            errors.add(error_or_warning);
            error_or_warning.clear();
            errorCounter++;
        }
        //JSON-LD consideration: The top level resource in the response must have the @context property, and it should appear as the very first key/value pair of the JSON representation
        if(!"".equals(context) && !"".equals(type)){
            //No need to proceed, this is bad and breaks all types.  All resources types must have @type and an @context must be defined for the top level object.  
            error_or_warning.element("error_"+errorCounter, "Either @type or @context was not found.");
            errors.add(error_or_warning);
            error_or_warning.clear();
            errorCounter++;
        }
            ...
            if(version.equals("3")){
                ...
                //QUESTION: What is the difference between version 2.0 and 2.1.1?  That may determine what you check for below...
                switch(type){ //2.0 - 2.1.1 spec
                    case "sc:Collection":
                        //Must have at least one label
                        //Should have one or more Metadata pairs
                        //Should have one or more description
                        //Should have exactly one thumbnail image, may have more than one
                        //May have one or more attribution labels
                        //May have one or more licenses
                        //May have one or more logos
                        //Must have exactly one @id and it must be the http(s) URI at which it is published.
                        //Must not have a format
                        //Must not have a height
                        //Must not have a width
                        //Must not have a viewing direction
                        //May have one or more viewing hints
                        //May have exactly one navigation date associated with it.
                        //May have one or more external resources related to it.
                        //May have one or more external rendering resources.
                        //May may have one or more links to an external service.
                        //May have one or more external descriptions related to it. (seeAlso)
                        //If it serves as pages must be within exactly one paged resource otherwise May be within one or more containing resource.
                        //Must not have a start canvas
                        //Must not have a content layer.     
                        //May have exactly one collection as its first page.
                        //May have exactly one collection as its last page.
                        //May have exactly one collection as its next page.
                        //May have exactly one collection as its prev page.
                        //May have exactly one startIndex, which must be the index of its first collection or manifest relative to the order established by its paging collection
                    ...
                    case "sc:Manifest":
                        ...
                        //Must not have a height
                        //Must not have a width
                        //May have exactly one viewing direction, and if so, it applies to all of its sequences unless the sequence specifies its own viewing direction.
                        //May have one or more viewing hints
                        //May have exactly one navigation date associated with it.
                        //May have one or more external resources related to it.
                        //May have one or more external rendering resources.
                        //May may have one or more links to an external service.
                        //May have one or more external descriptions related to it. (seeAlso)
                        //May be within one or more containing resource.
                        //Must not have a start canvas
                        //Must not have a content layer.     
                        //Must not have a first page.
                        //Must not have a last page.
                        //Must not have a next page.
                        //Must not have a prev page.                   
                        //Must not have a startIndex.

                    break;
                    case "sc:Sequence":
                        //May have one or more labels
                        //May have one or more Metadata pairs
                        //May have one or more description
                        //May have one or more thumbnail and should have at least one thumbnail if there are multiple sequences in a given Manifest
                        //May have one or more attribution labels
                        //May have one or more licenses
                        //May have one or more logos
                        //May have exactly one @id
                        //Must not have a format
                        //Must not have a height
                        //Must not have a width
                        //May have exactly one viewing direction
                        //May have one or more viewing hints
                        //Must not have a navigation date associated with it.
                        //May have one or more external resources related to it.
                        //May have one or more external rendering resources.
                        //May may have one or more links to an external service.
                        //May have one or more external descriptions related to it. (seeAlso)
                        //May be within one or more containing resource.
                        //May have exactly one canvas as its start canvas.
                        //Must not have a content layer.     
                        //Must not have a first page.
                        //Must not have a last page.
                        //Must not have a next page.
                    ...
                    case "sc:Canvas":
                        ...
                        //May have one or more Metadata pairs
                        //May have one or more description
                        //May have one or more thumbnail and should have at least one thumbnail if there are multiple images or resources that make up the representation
                        //May have one or more attribution labels
                        //May have one or more licenses
                        //May have one or more logos
                        //Must have exactly one @id and it must be an http(s) URI.  Tehe JSON representation should be published at the URI.
                        //Must not have a format
                        //Must have exactly one height.
                        //Must have exactly one width.
                        //Must not have a viewing direction
                        //May have one or more viewing hints
                        //Must not have a navigation date associated with it.
                        //May have one or more external resources related to it.
                    ...
                    case "sc:Range":
                        ...
                        //May have one or more Metadata pairs
                        //May have one or more description
                        //May have one or more thumbnails
                        //May have one or more attribution labels
                        //May have one or more licenses
                        //May have one or more logos
                        //Must have exactly one id, and it must be an http(s) URI.
                        //Must not have a format
                        //May have exactly one viewing direction
                        //Must not have a viewing direction
                        //May have one or more viewing hints
                        ...
                        //May be within one or more containing resource.
                        //May have exactly one canvas as its start canvas.
                        //May have exactly one layer as its content layer.
                        //Must not have a first page.
                        //Must not have a last page.
                        //Must not have a next page.
                        //Must not have a prev page.
                    ...
                    case "sc:AnnotationList":
                        ...
                        //Must not have a next page.
                        //Must not have a prev page.
                        //May have exactly one startIndex, which must be the index of its first annotation relative to the order established by its paging layer
                    ...
                    case "oa:Annotation":
                        ...
                        //May have one or more licenses
                        //May have one or more logos
                        //Should have exactly one id, Must not have more than one, and the annotation’s representation Should be published at that URI.
                        //Must not have a format
                        //Must not have a height
                        //Must not have a width
                        //Must not have a viewing direction
                    ...
                    case "sc:Layer":
                        ...
                        //Must not have a start canvas
                        //Must not have a content layer.     
                        //May have exactly one layer as its first page.
                        //May have exactly one layer as its last page.
                        //May have exactly one layer as its next page.
                        //May have exactly one layer as its prev page.
                        //Must not have a startIndex.

                    break;
                    default:
                        //This is an error, it is a type that is not supported by IIIF
                }
                //Don't forget about content resources inside these types
                //May have labels
                //Must have exactly one id unless it is embedded in the response, and it must be the http(s) URI at which the resource is published
                //May have exactly one format, and if so, it must be the value of the Content-Type header returned when the resource is dereferenced
                //May have exactly one height, given in pixels, if appropriate.
                //May have exactly one height, given in pixels, if appropriate.
            }
            else if(version.equals("1")){
                //TODO spec this out like for version 2
                
            }
            else{ // This is something custom
                //Not sure what to do here, probably fail with an error.
