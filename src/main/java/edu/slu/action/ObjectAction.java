 ...
 *         This implementation follows RESTFUL standards. If you make changes,
 *         please adhere to this standard.
 */
public class ObjectAction extends ActionSupport implements ServletRequestAware, ServletResponseAware {
     ...
     * 
     * @param refreshToken The refresh token given to the user on registration or
     ...
     * 
     * @FIXME We need to rethink what update.action does and how to separate and
     *        handle PUT vs PATCH gracefully and compliantly.
     * @param jo the JSON or JSON-LD object
     * @see getAnnotationByObjectID(),saveNewObject(),updateObject()
     * @return containerType Boolean representing if RERUM knows whether it is a
     *         container type or not.
     */
    private Boolean isContainerType(JSONObject jo) {
        ...
        // These are the types RERUM knows and IIIF says these types are containers. How
        // can we check against custom @context and types?
    ...
    /**
     * Write error to response.out. The methods that call this function handle
     * quitting, this just writes the error because of the quit.
     * 
    ...
    /**
     * Add the __rerum properties object to a given JSONObject.If __rerum already
     * exists, it will be overwritten because this method is only called on new
     * objects. Properties for consideration are:
     * APIversion —1.0.0
     ...
     * 
     * @param received A potentially optionless JSONObject from the Mongo Database
     *                 (not the user). This prevents tainted __rerum's
     * @param update   A trigger for special handling from update actions
     * @return configuredObject The same object that was recieved but with the
     *         proper __rerum options. This object is intended to be saved as a new
     *         object (@see versioning)
     */
    public JSONObject configureRerumOptions(JSONObject received, boolean update, boolean extUpdate) {
        ...
        try {
            ...
            // otherwise, it is a new save or an update on an object without the __rerum
            // property
        ...
        if (extUpdate) {
            // We are "importing" an external object as a new object in RERUM (via an
            // update). It can knows its previous external self, but is a root for its
            // existence in RERUM.
            received_options = new JSONObject();
            history_prime = "root";
            ...
            if (received_options.containsKey("history")) {
                ...
                if (update) {
                    // This means we are configuring from the update action and we have passed in a
                    // clone of the originating object (with its @id) that contained a
                    // __rerum.history
                    if (history.getString("prime").equals("root")) {
                        // Hitting this case means we are updating from the prime object, so we can't
                        // pass "root" on as the prime value
                        history_prime = received.getString("@id");
                    } else {
                        // Hitting this means we are updating an object that already knows its prime, so
                        // we can pass on the prime value
                        history_prime = history.getString("prime");
                    }
                    // Either way, we know the previous value shold be the @id of the object
                    // received here.
                    history_previous = received.getString("@id");
                } else {
                    // Hitting this means we are saving a new object and found that __rerum.history
                    // existed. We don't trust it, act like it doesn't have it.
                    history_prime = "root";
                    history_previous = "";
                }
            } else {
                // Hitting this means we are are saving an object that did not have __rerum
                // history. This is normal
                history_prime = "root";
                history_previous = "";
            }
        ...
        configuredObject.element("__rerum", rerumOptions); // .element will replace the __rerum that is there OR create
                                                           // a new one
        return configuredObject; // The mongo save/update has not been called yet. The object returned here will
                                 // go into mongo.save or mongo.update
    }

    /**
     * Internal helper method to update the history.previous property of a root
     * object. This will occur because a new root object can be created
     * by put_update.action on an external object. It must mark itself as root and
     * contain the original ID for the object in history.previous.
     * This method only receives reliable objects from mongo.
     * 
     * @param newRootObj    the RERUM object whose history.previous needs to be
     *                      updated
     * @param externalObjID the @id of the external object to go into
     *                      history.previous
     * @return JSONObject of the provided object with the history.previous
     *         alteration
     */
    private JSONObject alterHistoryPrevious(JSONObject newRootObj, String externalObjID) {
        DBObject myAnnoWithHistoryUpdate;
        DBObject myAnno = BasicDBObject.parse(newRootObj.toString());
        try {
            newRootObj.getJSONObject("__rerum").getJSONObject("history").element("previous", externalObjID); // write
                                                                                                             // back to
                                                                                                             // the anno
                                                                                                             // from
                                                                                                             // mongo
            myAnnoWithHistoryUpdate = BasicDBObject.parse(newRootObj.toString()); // make the JSONObject a DB object
            mongoDBService.update(Constant.COLLECTION_ANNOTATION, myAnno, myAnnoWithHistoryUpdate); // update in mongo
        } catch (Exception e) {
            writeErrorResponse(
                    "This object does not contain the proper history property.  It may not be from RERUM, the update failed.",
    ...
    /**
     * Internal helper method to update the history.next property of an object. This
     * will occur because updateObject will create a new object from a given object,
     * and that
     * given object will have a new next value of the new object. Watch out for
     * missing __rerum or malformed __rerum.history
     * 
     * @param idForUpdate the @id of the object whose history.next needs to be
     *                    updated
     * @param newNextID   the @id of the newly created object to be placed in the
     *                    history.next array.
     * @return Boolean altered true on success, false on fail
     */
    private boolean alterHistoryNext(String idForUpdate, String newNextID) {
        // TODO @theHabes As long as we trust the objects we send to this, we can take
        // out the lookup and pass in objects as parameters
        ...
        if (null != myAnno) {
            try {
                annoToUpdate.getJSONObject("__rerum").getJSONObject("history").getJSONArray("next").add(newNextID); // write
                                                                                                                    // back
                                                                                                                    // to
                                                                                                                    // the
                                                                                                                    // anno
                                                                                                                    // from
                                                                                                                    // mongo
                myAnnoWithHistoryUpdate = BasicDBObject.parse(annoToUpdate.toString()); // make the JSONObject a DB
                                                                                          // object
                mongoDBService.update(Constant.COLLECTION_ANNOTATION, myAnno, myAnnoWithHistoryUpdate); // update in
                                                                                                        // mongo
                altered = true;
            } catch (Exception e) {
    ...
    /**
     * Given an Authorization:Bearer {token} header, pull out the {token} and return
     * it.
     * 
     * @param authorizationHeader The 'Bearer {token}' String value to pull the
     *                            {token} from. If it does not contain 'Bearer', we
     *                            have a bad header.
    ...
    private String getTokenFromHeader(String authorizationHeader) {
        ...
        if (authorizationHeader.contains("Bearer")) {
            ...
            // Bad Authorization header. How should we handle? Right now, the token will be
     ...
     * is and checks the the method is appropriately RESTful. Returns false if not
     * and
    ...
    public Boolean methodApproval(HttpServletRequest http_request, String request_type) throws Exception {
        ...
        switch (request_type) {
            case "overwrite":
                ...
                if (auth_verified) {
                    ...
                    if ("".equals(access_token)) {
                        ...
                        writeErrorResponse("Could not authorize you to perform this action.  Have you registered at "
                                + Constant.RERUM_PREFIX, HttpServletResponse.SC_UNAUTHORIZED);
                    }
                }
                break;
            case "update":
                ...
                if (auth_verified) {
                    if (requestMethod.equals("PUT")) {
                        ...
                        writeErrorResponse(
                                "Improper request method for updating, please use PUT to replace this object.",
                                HttpServletResponse.SC_METHOD_NOT_ALLOWED);
                    }
                } else {
                    if ("".equals(access_token)) {
            ...
            case "patch":
                /**
                 * Note that PATCH is not a standard method. Sometimes, programming languages
                 * don't have support for the method and
                 * throw runtime errors, which forces people into using POST with a HTTP Method
                 * Override Header to say PATCH.
                 * As a result, the API must support catching this header on these POST requests
                 * and allowing it to be treated
                 * as PATCH throughout, which we can control here for patch, set and unset
                 * cases.
                 */
                auth_verified = verifyAccess(access_token);
                if (auth_verified) {
                    if (requestMethod.equals("PATCH")) {
                        ...
                        if (override.equals("yes")) {
                            ...
                            writeErrorResponse(
                                    "Improper request method for updating, PATCH to remove keys from this RERUM object.",
                                    HttpServletResponse.SC_METHOD_NOT_ALLOWED);
                        } else if (override.equals("improper")) {
                            // Error response returned by checkPatchOverrideSupport, don't double up and put
                            // one here
                        }
                    }
                } else {
            ...
            case "release":
                ...
                if (auth_verified) {
                    if (requestMethod.equals("PATCH")) {
                        ...
                        writeErrorResponse(
                                "Improper request method for updating, please use PATCH to alter this RERUM object.",
                                HttpServletResponse.SC_METHOD_NOT_ALLOWED);
                    }
                } else {
                    if ("".equals(access_token)) {
            ...
            case "create":
                ...
                if (auth_verified) {
                    if (requestMethod.equals("POST")) {
                        ...
                        writeErrorResponse("Improper request method for creating, please use POST.",
                                HttpServletResponse.SC_METHOD_NOT_ALLOWED);
                    }
                } else {
                    if ("".equals(access_token)) {
            ...
            case "delete":
                ...
                if (auth_verified) {
                    if (requestMethod.equals("DELETE")) {
                        ...
                        writeErrorResponse("Improper request method for deleting, please use DELETE.",
                                HttpServletResponse.SC_METHOD_NOT_ALLOWED);
                    }
                } else {
                    if ("".equals(access_token)) {
            ...
            case "get":
                ...
                if (requestMethod.equals("GET") || requestMethod.equals("HEAD")) {
                    ...
                    writeErrorResponse(
                            "Improper request method for reading, please use GET or request for headers with HEAD.",
                            HttpServletResponse.SC_METHOD_NOT_ALLOWED);
                }
                break;
            case "token":
                ...
                if (requestMethod.equals("POST")) {
                    ...
                    writeErrorResponse("Improper request method for the Auth0 proxy, please use POST.",
                            HttpServletResponse.SC_METHOD_NOT_ALLOWED);
                }
                break;
            case "getProps":
                // This is a getByProperties request, so it acts like a GET, but has body like
                // POST (and putting body in it forces POST method even when GET is set).
                auth_verified = true;
                if (requestMethod.equals("POST")) {
                    restful = true;
                } else {
                    writeErrorResponse(
                            "Improper request method for requesting objects with matching properties.  Use POST.",
                            HttpServletResponse.SC_METHOD_NOT_ALLOWED);
                }
                break;
            default:
                writeErrorResponse("Improper request method for this type of request (unknown).",
                        HttpServletResponse.SC_METHOD_NOT_ALLOWED);
        }
    ...
    /**
     * Since programming languages with HTTP packages don't all support PATCH, we
     * have to detect the workaround.
     ...
     * DELETE is a special case because the content could be JSON or just the @id
     * string and it only has to specify a content type if passing a JSONy object.
     ...
     * @param supportStringID The request may be allowed to pass the @id as the
     *                        body.
    ...
    public String processRequestBody(HttpServletRequest http_request, boolean supportStringID)
        ...
        if (null != cType && (cType.contains("application/json") || cType.contains("application/ld+json"))) {
            ...
            try {
                ...
                if (supportStringID) {
                    // We do not allow arrays of ID's for DELETE, so if it failed JSONObject parsing
                    // then this is a hard fail for DELETE.
                    ...
                    // Maybe it was an action on a JSONArray, check that before failing JSON parse
                    // test.
                    try {
                        // JSONArray test
                ...
                try {
                    ...
                    if (test.containsKey("@id")) {
                        ...
                        if ("".equals(requestBody)) {
                            ...
                            // This string could be ANYTHING. ANY string is valid at this point. Create a
                            // wrapper JSONObject for elegant handling in deleteObject().
                            // We will check against the string for existing objects in deleteObject(),
                            // processing the body is completed as far as this method is concerned.
                    ...
                    if ("".equals(requestBody)) {
                        // No ID provided
                        writeErrorResponse("Must provide an id or a JSON object containing @id of object to delete.",
                                HttpServletResponse.SC_BAD_REQUEST);
                        requestBody = null;
                    } else {
                        // This string could be ANYTHING. ANY string is valid at this point. Create a
         ...
         * writeErrorResponse(complianceInfo.toString(),
         * HttpServletResponse.SC_CONFLICT);
         * requestBody = null;
         * }
         * }
        ...
        input.close();
        content = requestBody;
        response.setContentType("application/json; charset=utf-8"); // We create JSON objects for the return body in
                                                                    // most cases.
        response.setHeader("Access-Control-Allow-Headers", "*");
        response.setHeader("Access-Control-Allow-Methods", "GET,OPTIONS,HEAD,PUT,PATCH,DELETE,POST"); // Must have
                                                                                                      // OPTIONS for
                                                                                                      // @webanno
        response.setHeader("Access-Control-Expose-Headers", "*");
    ...
    /**
     * Creates and appends headers to the HTTP response required by Web Annotation
     * standards.
     * Headers are attached and read from {@link #response}.
     * 
     * @param etag            A unique fingerprint for the object for the Etag
     *                        header.
     * @param isContainerType A boolean noting whether or not the object is a
     *                        container type.
     * @param isLD            the object is either plain JSON or is JSON-LD
     *                        ("ld+json")
     */
     ...
     * This is specifically for responses that are not Web Annotation compliant
     * (getByProperties, getAllDescendants(), getAllAncestors()).
    ...
    private void addSupportHeaders(String etag, Boolean isLD) {
        ...
        response.setHeader("Link",
                "<http://store.rerum.io/v1/context.json>; rel=\"http://www.w3.org/ns/json-ld#context\"; type=\"application/ld+json\"");
        response.setHeader("Allow", "GET,OPTIONS,HEAD,PUT,PATCH,DELETE,POST");
        if (!"".equals(etag)) {
            response.setHeader("Etag", etag);
        }
    ...
    /**
     * Public facing servlet action to find all upstream versions of an object. This
     * is the action the user hits with the API.
    ...
    public void getAllAncestors() throws Exception {
        if (null != oid && methodApproval(request, "get")) {
            ...
            BasicDBObject mongo_obj = (BasicDBObject) mongoDBService.findOneByExample(Constant.COLLECTION_ANNOTATION,
                    query);
            if (null != mongo_obj) {
                JSONObject safe_received = JSONObject.fromObject(mongo_obj); // We can trust this is the object as it
                                                                             // exists in mongo
                List<DBObject> ls_versions = getAllVersions(safe_received);
                JSONArray ancestors = JSONArray
                        .fromObject(getAllAncestors(ls_versions, safe_received, new JSONArray()));
                ...
                try {
                    ...
                    response.setHeader("Access-Control-Allow-Origin", "*");
                    response.setHeader("Access-Control-Expose-Headers", "*"); // Headers are restricted, unless you
                                                                              // explicitly expose them. Darn Browsers.
                    response.setStatus(HttpServletResponse.SC_OK);
                    out = response.getWriter();
    ...
    /**
     * Internal method to filter ancestors upstream from `key object` until `root`.
     * It should always receive a reliable object, not one from the user.
     * This list WILL NOT contains the keyObj.
     * 
     * "Get requests can't have body"
     * In fact in the standard they can (at least nothing says they can't). But lot
     * of servers and firewall implementation suppose they can't
     ...
     * @param keyObj              The object from which to start looking for
     *                            ancestors. It is not included in the return.
     * @param discoveredAncestors The array storing the ancestor objects discovered
     *                            by the recursion.
     * @return array of objects
     */
    private JSONArray getAllAncestors(List<DBObject> ls_versions, JSONObject keyObj, JSONArray discoveredAncestors) {
        String previousID = keyObj.getJSONObject("__rerum").getJSONObject("history").getString("previous"); // The first
                                                                                                            // previous
                                                                                                            // to look
                                                                                                            // for
        String rootCheck = keyObj.getJSONObject("__rerum").getJSONObject("history").getString("prime"); // Make sure the
                                                                                                        // keyObj is not
                                                                                                        // root.
        // @cubap @theHabes #44. What if obj does not have __rerum or __rerum.history
        for (DBObject thisVersion : ls_versions) {
            ...
            if ("root".equals(rootCheck)) {
                // Check if we found root when we got the last object out of the list. If so, we
                // are done. If keyObj was root, it will be detected here. Break out.
                break;
            } else if (thisObject.getString("@id").equals(previousID)) {
                // If this object's @id is equal to the previous from the last object we found,
                // its the one we want. Look to its previous to keep building the ancestors
                // Array.
                ...
                if ("".equals(previousID) && !"root".equals(rootCheck)) {
                    ...
                    // @cubap Yikes this is a problem. This branch on the tree is broken...what
                    // should we tell the user? How should we handle?

                    break;
                } else {
                    discoveredAncestors.add(thisObject);
                    // Recurse with what you have discovered so far and this object as the new
                    // keyObj
    ...
    /**
     * Public facing servlet to gather for all versions downstream from a provided
     * `key object`.
     * 
     * @param oid variable assigned by urlrewrite rule for /id in urlrewrite.xml
     * @throws java.lang.Exception
        ...
        if (null != oid && methodApproval(request, "get")) {
            ...
            if (null != mongo_obj) {
                JSONObject safe_received = JSONObject.fromObject(mongo_obj); // We can trust this is the object as it
                                                                             // exists in mongo
                List<DBObject> ls_versions = getAllVersions(safe_received);
                JSONArray descendants = JSONArray
                        .fromObject(getAllDescendants(ls_versions, safe_received, new JSONArray()));
    ...
    /**
     * Internal method to find all downstream versions of an object. It should
     * always receive a reliable object, not one from the user.
     * If this object is the last, the return will be an empty JSONArray. The keyObj
     * WILL NOT be a part of the array.
     * 
     * @param ls_versions           All the given versions, including root, of a
     *                              provided object.
     * @param keyObj                The provided object
     * @param discoveredDescendants The array storing the descendants objects
     *                              discovered by the recursion.
     * @return All the objects that were deemed descendants in a JSONArray
     */

    private JSONArray getAllDescendants(List<DBObject> ls_versions, JSONObject keyObj,
            JSONArray discoveredDescendants) {
        ...
        if (keyObj.getJSONObject("__rerum").getJSONObject("history").getJSONArray("next").isEmpty()) {
            ...
            // The provided object has nexts, get them to add them to known descendants then
    ...
    /**
     * Internal private method to loads all derivative versions from the `root`
     * object. It should always receive a reliable object, not one from the user.
     * Used to resolve the history tree for storing into memory.
     * 
     * @param obj A JSONObject to find all versions of. If it is root, make sure to
     *            prepend it to the result. If it isn't root, query for root from
     *            the ID
    ...
    private List<DBObject> getAllVersions(JSONObject obj) throws Exception {
        ...
        if (obj.getJSONObject("__rerum").getJSONObject("history").getString("prime").equals("root")) {
            ...
            // This is called by getAllAncestors and getAllDescendants which is why they do
            // not expandPrivateRerumProperty for their return.
            rootObj.remove("_id");
    ...
    public boolean checkModified(HttpServletRequest req, JSONObject jo) {
        if (request.getHeader("If-Modified-Since") != null && !request.getHeader("If-Modified-Since").equals("")) {
            // This is either isOverwritten or createdAt. Stuff we do manually in mongo
            // shell is not tracked, so make sure max-age is appropriate.
            ...
            if (jo.has("__rerum")) {
                ...
                if (lastModifiedDateObj == null) {
                    // System.out.println("Could not determine date from object, consider it
                    // modified");
            ...
            try {
                // Note that dates like 2021-05-26T10:39:19.328 have been rounded to
                // 2021-05-26T10:39:19.328 in browser headers. Account for that here.
                if (lastModifiedDateObj.contains(".")) {
                    // If-Modified-Since and Last-Modified headers are rounded. Wed, 26 May 2021
                    // 10:39:19.629 GMT becomes Wed, 26 May 2021 10:39:19 GMT.
                    lastModifiedDateObj = lastModifiedDateObj.split("\\.")[0];
                }
                ZonedDateTime fromHeader = ZonedDateTime.parse(lastModifiedDateHeader,
                ...
                // Note the date on v1/id/11111 like '1532026503859' breaks this. Not sure what
                // kind of back support would need to be available...
                System.out.println(
                        "Last-Modified Header could not be formed.  Bad date value ' " + lastModifiedDateObj + " '");
                return true;
            } catch (Exception e) {
                return true;
            }
            ...
            // System.out.println("No 'If-Modified-Since' Header present, consider it
            // modified");
            return true;
        }
    }
    ...
    /**
     * Get annotation by objectiD. Strip all unnecessary key:value pairs before
     * returning.
     * 
     * @param oid variable assigned by urlrewrite rule for /id in urlrewrite.xml
     * @rspond with the new annotation ID in the Location header and the new object
     *         created in the body.
     * @throws java.io.IOException
     * @throws javax.servlet.ServletException
     */
    public void getByID() throws IOException, ServletException, Exception {
        ...
        response.setHeader("Access-Control-Allow-Origin", "*");
        response.setHeader("Access-Control-Allow-Headers", "*");
        response.setHeader("Access-Control-Expose-Headers", "*"); // Headers are restricted, unless you explicitly
                                                                  // expose them. Darn Browsers.
        if (null != oid && methodApproval(request, "get")) {
            ...
            if (null != myAnno) {
                ...
                if (checkModified(request, jo)) {
                    // The following are rerum properties that should be stripped. They should be in
                    // __rerum.
                    jo.remove("_id");
                    jo.remove("addedTime");
                    ...
                    try {
                        ...
                        response.setHeader("Cache-Control", "max-age=86400, must-revalidate"); // Very little chance
                                                                                               // this has been
                                                                                               // overwritten, trust it
                                                                                               // for 24 hours
                        // This is either isOverwritten or createdAt. Stuff we do manually in mongo
                        // shell is not tracked, so make sure max-age is appropriate.
                        ...
                        if (jo.has("__rerum")) {
                            ...
                            try {
                                ...
                                response.setHeader("Last-Modified",
                                        fromObject.format(DateTimeFormatter.RFC_1123_DATE_TIME));
                            } catch (DateTimeParseException ex) {
                                // Note the date on v1/id/11111 like '1532026503859' breaks this. Not sure what
                                // kind of back support would need to be available...
                                System.out.println("Last-Modified Header could not be formed.  Bad date value ' "
                                        + lastModifiedDate + " '");
                            } catch (Exception e) {
                            ...
                            try {
                                ...
                                LocalDateTime dt = LocalDateTime.parse(lastModifiedDate,
                                        DateTimeFormatter.ISO_DATE_TIME);
                                ZonedDateTime dtFormat = dt.atZone(ZoneId.of("GMT"));
                                response.setHeader("Last-Modified",
                                        dtFormat.format(DateTimeFormatter.RFC_1123_DATE_TIME));
                            } catch (DateTimeParseException ex) {
                                // Note the date on http://devstore.rerum.io/v1/id/61f2eef2f8a1836453e30fcf like
                                // '1643311168146' breaks this. Not sure what kind of back support would need to
                                // be available...
                                System.out.println("Last-Modified Header could not be formed.  Bad date value ' "
                                        + lastModifiedDate + " '");
                            } catch (Exception e) {
                    ...
                    // FIXME we seem to make it here, but the response is still 200 with the object
                    // in the body...doesn't seem to save any time.
     ...
     * @reutrn list of annotations that match the given conditions. DO NOT RETURN
     *         DELETED OBJECTS
     */
    // This is not Web Annotation standard as the specifications states you respond
    // with a single object, not a list. Not sure what to do with these.
    // @cubap answer: I asked on oac-discuss and was told Web Annotation hasn't
    // handled lists yet, so just be nice.
    public void getByProperties() throws IOException, ServletException, Exception {
        ...
        if (null != processRequestBody(request, false) && methodApproval(request, "getProps")) {
            ...
            // https://docs.mongodb.com/v2.6/reference/operator/query/exists/
            ...
            if (null != lim) {
                ls_result = mongoDBService.findByExample(Constant.COLLECTION_ANNOTATION, query, Integer.parseInt(skip),
                        Integer.parseInt(lim));
            } else {
             ...
             * if(ls_result.size() > 0){
             * try {
             * response.setHeader("Content-Type","application/json"); // not ld+json because
             * it is an array
             * response.setHeader("Access-Control-Allow-Origin", "*");
             * response.setStatus(HttpServletResponse.SC_OK);
             * out = response.getWriter();
             ...
             * }
             * else{
             * writeErrorResponse("Object(s) not found using provided properties '"+received
    ...
    /**
     * TODO @see batchSaveMetadataForm. Do both methods need to exist? Combine if
     * possible. This is the method we use for generic bulk saving.
     * Each canvas has an annotation list with 0 - infinity annotations. A copy
     * requires a new annotation list with the copied annotations and a new @id.
     * Mongo allows us to bulk save.
     * The content is from an HTTP request posting in an array filled with
     * annotations to copy.
     * 
     * @throws java.io.UnsupportedEncodingException
     * @throws javax.servlet.ServletException
     * @see MongoDBAbstractDAO.bulkSaveFromCopy(String collectionName, BasicDBList
     *      entity_array);
     * @see MongoDBAbstractDAO.bulkSetIDProperty(String collectionName,
     *      BasicDBObject[] entity_array);
     */
    public void batchSaveFromCopy() throws UnsupportedEncodingException, IOException, ServletException, Exception {
        ...
        if (null != processRequestBody(request, false) && methodApproval(request, "create")) {
            ...
            for (int b = 0; b < received_array.size(); b++) { // Configure __rerum on each object
                ...
                received_array.set(b, configureMe); // Replace the current iterated object in the array with the
                                                    // configured object
            }
            BasicDBList dbo = (BasicDBList) JSON.parse(received_array.toString()); // tricky cause can't use JSONArray
                                                                                   // here
            JSONArray newResources = new JSONArray();
            // if the size is 0, no need to bulk save. Nothing is there.
            if (dbo.size() > 0) {
                // System.out.println("batch save from copy off to bulk save from copy
                // sending");
    ...
    public void saveNewObject() throws IOException, ServletException, Exception {
        ...
        if (null != processRequestBody(request, false) && methodApproval(request, "create")) {
            ...
            if (received.containsKey("@id") && !received.getString("@id").isEmpty()) {
                writeErrorResponse("Object already contains an @id " + received.containsKey("@id")
                        + ".  Either remove this property for saving or if it is a RERUM object update instead.",
                        HttpServletResponse.SC_BAD_REQUEST);
            } else {
                JSONObject iiif_validation_response = checkIIIFCompliance(received, true); // This boolean should be
                                                                                           // provided by the user
                                                                                           // somehow. It is a
                                                                                           // intended-to-be-iiif flag
                ...
                if (null != request.getHeader("Slug")) {
                    // Slug is the user suggested ID for the annotation. This could be a cool RERUM
                    // thing.
    ...
    public void patchSetUpdate() throws IOException, ServletException, Exception {
        ...
        if (null != processRequestBody(request, true) && methodApproval(request, "set")) {
            ...
            if (received.containsKey("@id")) {
                ...
                BasicDBObject updatedObject = (BasicDBObject) originalObject.copy(); // A copy of the original, this
                                                                                     // will be saved as a new object.
                                                                                     // Make all edits to this variable.
                ...
                if (alreadyDeleted) {
                    ...
                    if (null != originalObject) {
                        ...
                        int updateCount = 0;
                        // If the object already in the database contains the key found from the object
                        // recieved from the user...
                        for (String key : update_anno_keys) {
                            if (originalObject.containsKey(key)) { // Keys matched. Ignore it, set only works for new
                                                                   // keys.
                                // @cubap @theHabes do we want to build that this happened into the response at
                                // all?
                        ...
                        if (updateCount > 0) {
                            JSONObject newObject = JSONObject.fromObject(updatedObject);// The edited original object
                                                                                        // meant to be saved as a new
                                                                                        // object (versioning)
                            newObject = configureRerumOptions(newObject, true, false); // __rerum for the new object
                                                                                       // being created because of the
                                                                                       // update action
                            newObject.remove("@id"); // This is being saved as a new object, so remove this @id for the
                                                     // new one to be set.
                            // Since we ignore changes to __rerum for existing objects, we do no
                            // configureRerumOptions(updatedObject);
                            ...
                            mongoDBService.update(Constant.COLLECTION_ANNOTATION, dbo, dboWithObjectID);
                            historyNextUpdatePassed = alterHistoryNext(updateHistoryNextID, newNextAtID); // update
                                                                                                          // history.next
                                                                                                          // or original
                                                                                                          // object to
                                                                                                          // include the
                            ...
                            if (historyNextUpdatePassed) {
                                ...
                                jo.element("new_obj_state", newObject); // FIXME: @webanno standards say this should be
                                                                        // the response.
                                jo.element("iiif_validation", iiif_validation_response);
                                try {
                                    ...
                                    response.setHeader("Access-Control-Allow-Origin", "*");
                                    response.setHeader("Access-Control-Expose-Headers", "*"); // Headers are restricted,
        ...
        if (null != processRequestBody(request, true) && methodApproval(request, "patch")) {
            ...
            if (received.containsKey("@id")) {
                ...
                if (alreadyDeleted) {
                    ...
                    if (null != originalObject) {
                        BasicDBObject updatedObject = (BasicDBObject) originalObject.copy(); // A copy of the original,
                                                                                             // this will be saved as a
                                                                                             // new object. Make all
                                                                                             // edits to this variable.
                        Set<String> update_anno_keys = received.keySet();
                        boolean triedToSet = false;
                        int updateCount = 0;
                        // If the object already in the database contains the key found from the object
                        // recieved from the user, update it barring a few special keys
                        // Users cannot update the __rerum property, so we ignore any update action to
                        ...
                        if (triedToSet) {
                            ...
                            // @cubap @theHabes We continued with what we could patch. Do we tell the user
                            // at all?
                            // writeErrorResponse("A key you are trying to update does not exist on the
                            // object. You can set with the patch_set or put_update action.",
                            // HttpServletResponse.SC_BAD_REQUEST);
                        ...
                        writeErrorResponse(
                                "Object " + received.getString("@id") + " not found in RERUM, could not patch update.",
                                HttpServletResponse.SC_BAD_REQUEST);
                    }
                }
            } else {
    ...
    /**
     * Public facing servlet to PUT replace an existing object. Can set and unset
     * keys.
     * 
    ...
    public void putUpdateObject() throws IOException, ServletException, Exception {
        // @webanno The client should use the If-Match header with a value of the ETag
        // it received from the server before the editing process began,
        // to avoid collisions of multiple users modifying the same Annotation at the
        // same time
        // cubap: I'm not sold we have to do this. Our versioning would allow multiple
        // changes.
        ...
        if (null != processRequestBody(request, true) && methodApproval(request, "update")) {
            ...
            if (received.containsKey("@id")) {
                ...
                BasicDBObject originalObject = (BasicDBObject) mongoDBService
                        .findOneByExample(Constant.COLLECTION_ANNOTATION, query); // The originalObject DB object
                BasicDBObject updatedObject = (BasicDBObject) JSON.parse(received.toString()); // A copy of the
                                                                                               // original, this will be
                                                                                               // saved as a new object.
                                                                                               // Make all edits to this
                                                                                               // variable.
                ...
                if (alreadyDeleted) {
                    ...
                    if (null != originalObject) {
                        JSONObject newObject = JSONObject.fromObject(updatedObject);// The edited original object meant
                        ...
                        // provided by the user. We get a reliable copy oof the original out of mongo
                        newObject = configureRerumOptions(newObject, true, false); // __rerum for the new object being
                                                                                   // created because of the update
                                                                                   // action
                        newObject.remove("@id"); // This is being saved as a new object, so remove this @id for the new
                                                 // one to be set.
                        newObject.remove("_id");
                        DBObject dbo = (DBObject) JSON.parse(newObject.toString());
                        String newNextID = mongoDBService.save(Constant.COLLECTION_ANNOTATION, dbo);
                        ...
                        if (historyNextUpdatePassed) {
                            ...
                            jo.element("code", HttpServletResponse.SC_OK);
                            jo.element("original_object_id", updateHistoryNextID);
                            jo.element("new_obj_state", newObject); // FIXME: @webanno standards say this should be the
                                                                    // response.
                            jo.element("iiif_validation", iiif_validation_response);
                            try {
                                addWebAnnotationHeaders(newNextID, isContainerType(newObject), isLD(newObject));
    ...
    /**
     * Public facing servlet to PUT overwrite an existing object. Can set and unset
     * keys. There will be no reference to the node as it originally existed
     * because this intentionally avoids any versioning around the action. It is a
     * pure overwrite and should only be allowed for the generator of the object.
     * Do NOT overwrite __rerum, keep the original no matter what __rerum is
     * provided by the user.
     * 
     * 
     * @respond with new state of the object in the body.
        ...
        if (null != processRequestBody(request, true) && methodApproval(request, "overwrite")) {
            ...
            if (received.containsKey("@id")) {
                ...
                if (alreadyDeleted) {
                    ...
                    if (null != originalObject) {
                        JSONObject newObject = received;// The edited original object meant to be saved as a new object
                                                        // (versioning)
                        newObject.remove("_id");
                        JSONObject originalProperties = originalJSONObj.getJSONObject("__rerum");
                        LocalDateTime dt = LocalDateTime.now();
                        ...
                        try {
                            ...
                            addLocationHeader(newObject);
                            response.setHeader("Access-Control-Allow-Origin", "*");
                            response.setHeader("Access-Control-Expose-Headers", "*"); // Headers are restricted, unless
    ...
    public void releaseObject() throws IOException, ServletException, Exception {
        ...
        if (null != processRequestBody(request, true) && methodApproval(request, "release")) {
            ...
            if (received.containsKey("@id")) {
                ...
                BasicDBObject releasedObject = (BasicDBObject) originalObject.copy(); // A copy of the original. Make
                                                                                      // all edits to this variable.
                JSONObject safe_original = JSONObject.fromObject(originalObject); // The original object represented as
                                                                                  // a JSON object. Safe for edits.
                ...
                if (alreadyReleased) {
                    ...
                    if (null != originalObject) {
                        ...
                        if (isGenerator) {
                            ...
                            if (!"".equals(previousReleasedID)) {// A releases tree exists and an ancestral object is
                                                                 // being released.
                                treeHealed = healReleasesTree(safe_original);
                            } else { // There was no releases previous value.
                                if (nextReleases.size() > 0) { // The release tree has been established and a descendant
                                                               // object is now being released.
                                    treeHealed = healReleasesTree(safe_original);
                        ...
                        // This could mean it was an external object, but the release action fails on
                        // those.
                        writeErrorResponse(
    ...
    private boolean healReleasesTree(JSONObject releasingNode) throws Exception {
        ...
        for (int a = 0; a < anscestors.size(); a++) { // For each ancestor
            JSONArray ancestorNextArray = anscestors.getJSONObject(a).getJSONObject("__rerum").getJSONObject("releases")
                    .getJSONArray("next");
            ...
            if (origNextArray.size() == 0) {
                ...
                if (ancestorNextArray.size() == 0) {
                    ancestorNextArray.add(releasingNode.getString("@id")); // Add the id of the node I am releasing into
                                                                           // the ancestor's releases.next array.
                }
            } else {
                ...
                for (int i = 0; i < origNextArray.size(); i++) { // For each id in the next array of the object I am
                                                                 // releasing (will not be []).
                    String compareOrigNextID = origNextArray.getString(i);
                    for (int j = 0; j < ancestorNextArray.size(); j++) { // For each id in the ancestor's releases.next
                                                                         // array
                        ...
                        if (j == ancestorNextArray.size() - 1) { // Once I have checked against all id's in the ancestor
                                                                 // node releases.next[] and removed the ones I needed
                                                                 // to
                            ancestorNextArray.add(releasingNode.getString("@id")); // Add the id of the node I am
                                                                                   // releasing into the ancestor's
                                                                                   // releases.next array.
                        }
                    }
                }
    ...
    /**
     * A helper function that determines whether or not an object has been flagged
     * as deleted.
    ...
    /**
     * A public facing servlet to determine if a given object ID is for a deleted
     * object.
     * 
     * @FIXME make the parameter the http_request, get the @id, get from mongo and
     *        feed to private version.
     * @param obj_id
     * @return A boolean representing the truth.
     */
    ...
    /**
     * check that the API keys match and that this application has permission to
     * delete the object. These are internal and the objects passed in are
    ...
    private boolean checkApplicationPermission(JSONObject obj) {
        ...
        // @cubap @theHabes TODO check that the API keys match and that this application
        // has permission to delete the object
        return permission;
    }

    /**
     * A helper function that gathers an object by its id and determines whether or
     * not it is flagged as released. These are internal and the objects passed in
     * are
     * first taken from mongo, they are not the obj provided by the application.
     * 
     * @param obj
    ...
    /**
     * Public facing servlet that checks whether the provided object id is of a
     * released object.
     * 
     * @param obj_id
     * @return
    ...
    public void deleteObject() throws IOException, ServletException, Exception {
        ...
        if (null != processRequestBody(request, true) && methodApproval(request, "delete")) {
            ...
            // processRequestBody will always return a stringified JSON object here, even if
            // the ID provided was a string in the body.
            JSONObject received = JSONObject.fromObject(content);
            ...
            if (received.containsKey("@id")) {
                ...
                safe_received = JSONObject.fromObject(mongo_obj); // We can trust this is the object as it exists in
                                                                  // mongo
                ...
                if (passedAllChecks) { // If all checks have passed. If not, we want to make sure their
                                       // writeErrorReponse() don't stack.
                    originalObject = (BasicDBObject) JSON.parse(safe_received.toString()); // The original object out of
                                                                                           // mongo for persistance
                    ...
                    if (null != originalObject) {
                        ...
                        updatedWithFlag.element("__deleted", deletedFlag); // We want everything wrapped in deleted
                                                                           // except the @id.
                        Object forMongo = JSON.parse(updatedWithFlag.toString()); // JSONObject cannot be converted to
                                                                                  // BasicDBObject
                        ...
                        if (treeHealed) {
                            ...
                            // @cubap @theHabes FIXME By default, objects that don't have the history
                            // property will fail to this line.
                            writeErrorResponse("We could not update the history tree correctly.  The delete failed.",
    ...
    /**
     * An internal method to handle when an object is deleted and the history tree
     * around it will need amending.
    ...
    private boolean healHistoryTree(JSONObject obj) {
        ...
        try {
            ...
            success = false; // This will bubble out to deleteObj() and have the side effect that this object
                             // is not deleted. @see treeHealed
        }
        boolean objToDeleteisRoot = prime_id.equals("root");
        // Update the history.previous of all the next ids in the array of the deleted
        // object
        for (int n = 0; n < next_ids.size(); n++) {
            ...
            if (null != objToUpdate) {
                ...
                if (objToDeleteisRoot) {
                    // The object being deleted is root. That means this next object must become
                    // root.
                    ...
                    try {
                        ...
                        previous_id = ""; // A hack to make sure we do not process the history.previous b/c there was an
                                          // error.
                        success = false; // This is an error
                        break; // Stop updating things, there has been an error. This history.next[i] object
                               // cannot be considered prime.
                    }
                } else if (!previous_id.equals("")) { // The object being deleted had a previous. That is now absorbed
                                                      // by this next object to mend the gap.
                    fixHistory.getJSONObject("__rerum").getJSONObject("history").element("previous", previous_id);
                } else {
                    System.out.println("object did not have previous and was not root.  Weird...");
                    // @cubap @theHabes TODO Yikes this is some kind of error...it is either root or
                    // has a previous, this case means neither are true.
                    // cubap: Since this is a __rerum error and it means that the object is already
                    // not well-placed in a tree, maybe it shouldn't fail to delete?
                    // theHabes: Are their bad implications on the relevant nodes in the tree that
                    // reference this one if we allow it to delete? Will their account of the
                    // history be correct?
                    // success = false;
                }
                ...
                System.out.println(
                        "Cannot find object associated with the history.next[i] URI.  It is not in the RERUM database.  URI:"
                                + nextID);
                previous_id = ""; // A hack to make sure we do not process the history.previous b/c there was an
                                  // error.
                success = false;
                // Yikes this is an error, could not find an object assosiated with id found in
                // history tree.
                break;
            }
        }
        if (previous_id.contains(Constant.RERUM_PREFIX)) {
            // The object being deleted had a previous that is internal to RERUM. That
            // previous object next[] must be updated with the deleted object's next[].
            ...
            if (null != objToUpdate2) {
                ...
                // JSONArray does not have splice, but we can code our own. This will splice out
                // obj["@id"].
                for (int i = 0; i < origNextArray.size(); i++) {
                    if (!origNextArray.getString(i).equals(obj.getString("@id"))) {
                        // So long as the value is not the deleted id, add it to the newNextArray (this
                        // is the splice).
                ...
                fixHistory2.getJSONObject("__rerum").getJSONObject("history").element("next", newNextArray); // Rewrite
                                                                                                             // the
                                                                                                             // next[]
                                                                                                             // array to
                                                                                                             // fix the
                                                                                                             // history
                Object forMongo2 = JSON.parse(fixHistory2.toString()); // JSONObject cannot be converted to
                                                                       // BasicDBObject
                ...
                // It has this APIs id pattern, that means we expected to find it. This is an
                // error.
                System.out.println(
                        "Cannot find object associated with the history.previous URI.  It is not in the RERUM database.  URI:"
                                + previous_id);
                success = false;
            }
        } else {
            // System.out.println("The value of history.previous was an external URI or was
            // not present. Nothing to heal. URI:"+previous_id);
        }
        return success;
    }

    /**
     * An internal method to make all descendants of this JSONObject take on a new
     * history.prime = this object's @id
    ...
    /**
     * Validate data is IIIF compliant against IIIF's validator. This object is
     * intended for creation and not yet saved into RERUM so it does not yet have
     * an @id.
     * The only way to hit the IIIF Validation API is to use the object's @id. The
     * idea would be to save the objects to get an id, hit the
     * IIIF API and if the object was intended to be IIIF, delete it from the store
     * and return an error to the user.
     * 
     * In the case the object was not intended to be IIIF, do not return an error.
     * Since the methods calling this will handle what happens based of
     * iiif_return.okay,
     * just set okay to 1 and the methods calling this will treat it as if it isn't
     * a problem.
     * 
     * @param objectToCheck A JSON object to parse through and validate. This object
     *                      has not yet been saved into mongo, so it does not have
     *                      an @id yet
     * @param intendedIIIF  A flag letting me know whether or not this object is
     *                      intended to be IIIF. If it isn't, don't treat validation
     *                      failure as an error.
     * @return iiif_return The return JSONObject from hitting the IIIF Validation
     *         API.
     */
    public JSONObject checkIIIFCompliance(JSONObject objectToCheck, boolean intendedIIIF)
        ...
        mongoDBService.update(Constant.COLLECTION_ANNOTATION, dbo, dboWithObjectID);
        iiif_return = checkIIIFCompliance(uid, "2.1"); // If it is an object we are creating, this line means @context
                                                       // must point to Presentation API 2 or 2.1
        if (iiif_return.containsKey("okay")) {
            if (iiif_return.getInt("okay") == 0) {
                if (intendedIIIF) {
                    // If it was intended to be a IIIF object, then remove this object from the
                    // store because it was not valid and return an error to the user

                } else {
                    // Otherwise say it is ok so the action looking do validate does not
                    // writeErrorResponse()
            ...
            // Then the validator had a problem, perhaps it timed out and returned a blank
            // object...
    ...
    /**
     * A JSONObject that already has an @id can be validated against this IIIF
     * validation URL. It will return a JSONObject.
     * A save or update action could hit this to see if the resulting object is IIIF
     * valid. If not, a 'rollback' or 'delete' could be performed
     * and the warnings and errors sent back to the user. If using this function, it
     * is assumed objURL is intended to be a IIIF URL.
     * 
     * @param objURL  The @id or id URL of a IIIF JSON object represented as a
     *                String.
     * @param version The Intended Presentation API version to validate against
     *                represented as a String. (1, 2 or 2.1)
     * @return iiif_return The return JSONObject from hitting the IIIF Validation
     *         API.
    ...
    public JSONObject checkIIIFCompliance(String objURL, String version) throws MalformedURLException, IOException {
        JSONObject iiif_return = new JSONObject();
        String iiif_validation_url = "https://iiif.io/api/presentation/validator/service/validate?format=json&version="
                + version + "&url=" + objURL;
        ...
        // connection.setConnectTimeout(5*1000); //This tends to choke
        // sometimes...should i handle a timeout better?
        ...
        // }
        // }
        // catch(java.net.SocketTimeoutException e){ //This specifically catches the
        // timeout
        // System.out.println("The iiif endpoint is taking too long, so its going to be
        // a blank object.");
        // iiif_return = new JSONObject(); //We were never going to get a response, so
    ...
    /**
     * Internal helper method to handle put_update.action on an external object. The
     * goal is to make a copy of object as denoted by the PUT request
     * as a RERUM object (creating a new object) then have that new root object
     * reference the @id of the external object in its history.previous.
     * 
     * @param externalObj the external object as it existed in the PUT request to be
     *                    saved.
     */
    ...
    public static String getAccessTokenWithAuth(String auth_code) throws UnsupportedEncodingException {
        ...
        try {
            ...
            if (connection.getResponseCode() == 401) {
                // One of the values used was wrong. The secret may have expired, the clientID
                // may be wrong, the auth_code may be wrong or the user may not be a part of the
                // RERUM client.
    ...
    private boolean verifyAccess(String access_token) throws IOException, ServletException, Exception {
        ...
        try {
            ...
            // Could cache this so we don't have to keep grabbing it since it has to happen
            // on every call.
            ...
            // Cache the key so we don't have to keep requesting auth0 for it. Expires in
            // cache every 10 hours.
            if (null == pubKey) {
            ...
            System.out.println("claimKeys has length "+claimKeys.length);
            for (int i = 0; i < claimKeys.length - 1; i++) {
                ...
                if (!agent.isNull()) {
                    System.out.println("agent is not null this time");
                    break;
                }
            ...
            if (null != result) {
                System.out.println("[Modifying Data Request]: ip ========== " + requestIP + "@" + sdf.format(new Date())
                        + " +++++ App registered in legacy system");
                ...
                System.out.println("[Modifying Data Request]: ip ========== " + requestIP + "@" + sdf.format(new Date())
                        + " +++++ The app needs to register with RERUM");
                verified = false;
            }
        }
    ...
    private JSONObject generateAgentForLegacyUser(JSONObject legacyUserObj) {
        System.out
                .println("Detected a legacy registration.  Creating an agent and storing it with this legacy object.");
        ...
        mongoDBService.update(Constant.COLLECTION_ACCEPTEDSERVER, originalToUpdate, updatedOrig); // This update does
                                                                                                  // not appear to work
                                                                                                  // every time...
    ...
    private JSONObject expandPrivateRerumProperty(JSONObject thisObject) {
        // This is a breaking change for applications relying on the __rerum field
        // (TPEN_NL). Need to hold off on this for now.
